...
That bring the number of event categories down to 48 (from 985) and so makes this data set a lot more useful. It is still not in a state that we can make useful reports as such. Some summary data frames would be helpful to report and plot against.

#### Summarising Injuries and Fatalities ####

Firstly, we need to summarise the data into total counts of injuries and fatalities by the event type. This only does the totals per event type and does not break it down further into years. Providing a more granular summary may be worth doing for finer analysis by period (i.e. year) but is outside the scope of this assessment.
...
glimpse(event.health)
```

This has produced a clean data frame that we can use in our analysis.

#### Summarising Economic Cost ####

There are a number of steps that we need to undertake to be able to get the summary data required for plotting.

Firstly, we will get just a subset of the columns, needed from the masin dataframe 
...
We don't need the full date in this data set so lets extract just the "year" portion of it
...
The costs for both property and crops are broken down into two fields each, being a cost and a multiplication factor. This actually makes it harder to report on so we need to create a single column for each of the costs.

The first stage of this process is that we need to recode the factor columns so that they are more consistent.
...
As shown above, this has converted all of the K's,M's, 4's etc into an actual figure that can be used as a factor to multiply the short cost into a full cost.

Now we could immediately do the calculation of the costs, however this does not take into account inflation. The figures are raw only so at this stage 1950's figures will be significantly lower than those of 2011.

The way of overcoming this problem is to set up a set of factors for inflation. As the data we are working with finishes in 2011, Lets just work on factors for the period iun question to make all costs in 2011 dollar amounts.
...
Loopks god but of course it is no good just by itself... we need to merge it into the data frame that has the costs associated with it.
...
Now that we have all the elements in the dataframe in place, we simply need to run the calculations that will produce the costs in 2011 dollar terms.
...
We're are almost done now, all that is left to do is actually produce the summary data frame. Some of these numbers will be big so lets calculate the resulting costs to be in lots of $100,000.
...
glimpse(event.damage)
```


### Results ###

As a result of processing the data, we have two data sets which can be used for reporting and plotting summary information. These data sets are called:

<pre>
    <code>
        event.health        # Summary of Injuries and Fatalities by Event Type 1950-2011
        event.damage        # Summary of Economic Costs to Property and Crops by Event 1950-2011
    </code>
</pre>

#### Result of Injuries and Fatalities Processing ####

Lets firstly have a look at the summary data that wa produced as part of our processing above.

```{r}
event.health
```

It can be seen quite clearly that Tornados have by far the greatest impact on both Injuries and Fatlities across the US in the last 60+ years. The interesting part, particularly for injuries, is that Thunderstorm winds have the second highest impact, greater than fires, flooding and excess heat etc.



Lets see what the injury figures look like in a plot.
...
And also lets see how the fatalities compare in a separet plot. 
...
It would also be interesting to see what the mean, median of all injuries and fatalities are;

```{r}
summary(event.health)
```

And also what percentage fatalies are compared to injuries

```{r}
event.health %>%
    mutate(percent.fatality = fatalities / injuries * 100)
```

This does produce some surprises in that while there can be a wide gap, several have more significantly more fatalities than injuries, with Cold/Wind Chill being the highest at 263.3%. Another interesting point is that Sleet produced no injuries but had 2 fatalities during this period.

#### REsult of Economic Cost Processing ####

Firstly, lets inspect the summary data...

```{r}
event.damage
```

While the figures are hard to read from the table, it is clear that overall, property cost damages tend to be significantly higher than crop costs. Lets visualise this data set
...
This plot does indeed prove the fact that property cost are significantly higher crop costs caused by damage. Lets see what the overall statistics say between property and crop damage costs.

```{r}
summary(event.damage)
```

This shows that costs to crops is around 10% of the cost of damage to properties.
